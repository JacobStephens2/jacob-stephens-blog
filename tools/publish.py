...
`build` does exactly three things:
  ...
  3. Adds or removes an .htaccess in each post's directory, so a draft asks
     for the preview password and a published post never does.

It never touches anything else in a post. Draft posts stay on disk, but they
are no longer public: reaching one needs the shared preview password. Share
that with a reviewer and the URL works for them as before.

Because (3) is generated from posts.json, publishing a post lifts its
password gate in the same step. Never hand-write one of those files - a
stray gate would leave a published post behind a prompt, and `check` is what
catches that.
...
# Draft access. A draft is not public: its directory carries an .htaccess that
# demands the shared preview password. The password file lives outside the
# repository on purpose — this repo is public, so nothing secret may be
# committed, and this file holds only a path. If that password file is missing
# Apache refuses the request, which is the right way to fail.
DRAFT_HTPASSWD = "/var/www/.htpasswd-drafts"
DRAFT_HTACCESS = (
    "# Generated by tools/publish.py - do not edit.\n"
    "# This post is a draft. Publishing it removes this file.\n"
    "AuthType Basic\n"
    'AuthName "jacobstephens.net draft preview"\n'
    f"AuthUserFile {DRAFT_HTPASSWD}\n"
    "Require valid-user\n"
)
...
def build(write=True):
    ...
    for p in data["posts"]:
        ...
        gate = d / ".htaccess"
        want_draft = p["status"] != "published"
        correct = gate.exists() and gate.read_text() == DRAFT_HTACCESS
        if want_draft and not correct:
            changed.append(f"posts/{p['slug']}/.htaccess")
            if write:
                gate.write_text(DRAFT_HTACCESS)
        elif not want_draft and gate.exists():
            changed.append(f"posts/{p['slug']}/.htaccess (removed)")
            if write:
                gate.unlink()
    return changed
